...
const Hero = () => {
  ...
  return (
  <div className={`bg-[url('/photos/hero-temp.jpg')] bg-cover h-screen flex items-end w-full`}>
    ...
    <Section className='w-full' yPadding="py-16">
        <header>
          <Card className="flex flex-col lg:flex-row gap-3 lg:gap-6 justify-between p-3 md:p-4">
            ...
            <h3 className='text-2xl font-bold uppercase'>{width > breakpoint ? 'Book your next event with us!' : 'Book with us now!'}</h3>
...
<FormControl fullWidth>
  ...
  >
    {getFutureDates(new Date(), 14).map((el, i) => (
      <MenuItem key={`date-dropdown-${i}`} value={el}>{el}</MenuItem>
    ))}
  </Select>
</FormControl>     
              </div>
...
<Button className="whitespace-nowrap bg-blue-500 hover:bg-blue-600" variant='contained' size="large">
                Send Interest
              </Button>
              </div>
          </Card>
